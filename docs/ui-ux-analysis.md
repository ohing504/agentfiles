...
| 순위 | 제안 | 영향 | 난이도 |
|------|------|------|--------|
| ~~1~~ | ~~Agents/Commands/Skills 파일 내용 표시 구현~~ (완료) | 통합 Files 뷰에서 에디터로 직접 편집 가능하게 구현 완료 | 보통 |
| ~~2~~ | ~~CLAUDE.md 에디터 저장 성공 피드백~~ (완료) | 인라인 "Saved" 텍스트 + i18n 전환 완료 | 쉬움 |
| ~~3~~ | ~~개발 도구 프로덕션 제외~~ (완료) | `import.meta.env.DEV` 조건 래핑 완료 | 쉬움 |
| ~~4~~ | ~~MCP 다이얼로그 select 컴포넌트 교체~~ (완료) | shadcn Select로 교체 — 통합 Files 뷰 PR에서 완료 | 쉬움 |

### 중간 (UX 향상, 구현 난이도: 쉬움)

| 순위 | 제안 | 영향 | 난이도 |
|------|------|------|--------|
| ~~5~~ | ~~빈 상태 CTA 추가~~ (해결됨) | 통합 Files 뷰로 개별 빈 상태 페이지 삭제됨, 트리 UI로 대체 | 쉬움 |
| ~~6~~ | ~~Dashboard StatCard i18n 적용~~ (완료) | `m.scope_global()`/`m.scope_project()` 적용 완료 | 쉬움 |
| ~~7~~ | ~~Plugin 카드 클릭 영역 분리~~ (완료) | 제목만 Link로 변경, 토글 버튼 독립 — 통합 Files 뷰 PR에서 완료 | 쉬움 |
| ~~8~~ | ~~CLAUDE.md 경로 표시 중복 제거~~ (해결됨) | 기존 claude-md.tsx 삭제, 통합 Files 뷰에서 중복 없이 구현 | 쉬움 |
...
| ~~10~~ | ~~LanguageSwitcher 접근성 개선~~ (부분 완료) | `aria-pressed` 추가 완료, `aria-label`/shadcn Button은 미적용 | 쉬움 |
