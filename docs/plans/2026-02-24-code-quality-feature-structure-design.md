...
| 항목 | 결정 |
|------|------|
| 폴더 구조 | **Hybrid** — src/features/ 는 feature 기반, server/services/hooks는 기술 레이어 유지 |
...
- `src/components/pages/` — plugins, mcp, files (나중에 features/로 이동 예정)
- `src/components/settings/` — 설정 페이지
- `src/components/ui/` — shadcn 원본
- `src/components/*.tsx` — 공유 컴포넌트 (components/ 루트에 유지)
