...
- [ ] `tsconfig.json` — strict, paths(`@server/*`, `@web/*`, `@shared/*`)
- [ ] `vite.config.ts` — React plugin, 프록시(`/api` → `localhost:3001`), build output(`dist/web`)
- [ ] `tsup.config.ts` — entry: `src/server/index.ts` + `bin/cli.ts`, format: esm, outDir: `dist`
- [ ] `biome.json` — formatter + linter 규칙
- [ ] `.gitignore` 보완 — `dist/`, `node_modules/`
...
- [ ] `ClaudeMd` — scope, path, content, size, lastModified(string ISO)
- [ ] `Plugin` — id, name, marketplace, scope(`user`|`project`), version, enabled 등
- [ ] `McpServer` — name, scope, type(`stdio`|`sse`|`streamable-http`), command?, args?, url?, env?, disabled?
- [ ] `AgentFile` — name, scope, path, namespace?, frontmatter?, size, lastModified, type(`agent`|`command`|`skill`), isSymlink?, symlinkTarget?
- [ ] `Overview` — claudeMd, plugins, mcpServers, agents, commands, skills 카운트 + conflictCount
...
- [ ] Bearer 토큰 검증
- [ ] `/api/health` 등 공개 엔드포인트 화이트리스트
- [ ] 인증 실패 시 `401 { error: "Unauthorized" }`

### 3-3. `src/server/index.ts`
- [ ] Hono 앱 생성
- [ ] auth 미들웨어 등록
- [ ] 라우터 마운트
- [ ] `@hono/node-server`로 서버 시작
- [ ] 프로덕션 모드: `dist/web/` 정적 파일 서빙

### 3-4. `src/server/router.ts`
- [ ] 각 라우트 파일 통합
- [ ] RPC 타입 export (`AppType`)
...
- [ ] `getClaudeMd(scope)` — CLAUDE.md 읽기 (메타 + 내용)
  - 파일 없으면 null 반환
- [ ] `scanMdDir(basePath, type)` — agents/, commands/, skills/ 스캔
  - 재귀 탐색 (네임스페이스 폴더 대응)
  - gray-matter로 frontmatter 파싱
  - symlink 감지 (`fs.lstat`)
- [ ] `getPlugins()` — `~/.claude/plugins/installed_plugins.json` 파싱
  - 파일 없으면 빈 배열
- [ ] `getMcpServers()` — settings.json에서 MCP 서버 추출
  - stdio / sse / streamable-http 타입 구분
  - 글로벌 + 프로젝트 양쪽 스캔
- [ ] `getOverview()` — 위 함수들 조합하여 대시보드 데이터 집계
...
- [ ] `writeMarkdown(path, content)` — 마크다운 파일 저장
  - 디렉토리 없으면 자동 생성 (`mkdir -p`)
- [ ] `createFile(basePath, name, content, scope)` — 새 agent/command/skill 생성
- [ ] `deleteFile(path)` — 파일 삭제
- [ ] `renameFile(oldPath, newPath)` — 파일 이름 변경

### 5-2. `src/server/services/claude-cli.ts`

- [ ] `checkCliAvailable()` — `claude --version` 실행
  - 없으면 `{ available: false, reason: "..." }` 반환
- [ ] `mcpAdd(name, command, args, env, scope)` — `claude mcp add` 위임
  - scope에 따라 `-s user` 또는 `-s project` 플래그
- [ ] `mcpRemove(name, scope)` — `claude mcp remove` 위임
- [ ] `pluginToggle(id, enable)` — `claude plugin enable/disable` 위임
- [ ] 공통 에러 핸들링
...
- [ ] file-writer 단위 테스트 (tmp 디렉토리)
- [ ] claude-cli 단위 테스트 (mock child_process)
...
- [ ] `GET /api/overview` → `configService.getOverview()`

### 6-2. `src/server/routes/claude-md.ts`
- [ ] `GET /api/claude-md/:scope` → 메타 + 내용
- [ ] `PUT /api/claude-md/:scope` → `fileWriter.writeMarkdown()`

### 6-3. `src/server/routes/plugins.ts`
- [ ] `GET /api/plugins` → 전체 플러그인 목록
- [ ] `GET /api/plugins/:id` → 개별 플러그인 상세
- [ ] `PATCH /api/plugins/:id` → enable/disable (CLI 위임)
...
- [ ] `GET /api/agents` → 목록 (글로벌 + 프로젝트)
- [ ] `GET /api/agents/:name` → 상세 + 내용
- [ ] `POST /api/agents` → 생성
- [ ] `PUT /api/agents/:name` → 수정
- [ ] `DELETE /api/agents/:name?scope=` → 삭제

### 6-6. `src/server/routes/commands.ts`
- [ ] 동일 CRUD 패턴 (agents와 동일 구조)

### 6-7. `src/server/routes/skills.ts`
- [ ] 동일 CRUD 패턴 + symlink 정보 포함

### 6-8. CLI 상태 엔드포인트
- [ ] `GET /api/cli-status` → Claude CLI 설치 여부 + 버전
...
### 7-1. 기본 설정
- [ ] `src/web/main.tsx` — React 진입점, QueryClientProvider
- [ ] `src/web/App.tsx` — TanStack Router 설정
- [ ] Tailwind CSS v4 설정
- [ ] shadcn/ui 초기화 — Button, Card, Badge, Input, Dialog, Tabs, Textarea

### 7-2. 레이아웃
- [ ] `src/web/components/Layout.tsx` — 사이드바 + 메인 콘텐츠 영역
...
### 7-3. 공통 컴포넌트
- [ ] `src/web/components/ScopeBadge.tsx` — Global/Project 표시 + 충돌 경고
- [ ] `src/web/components/FileTree.tsx` — 트리 형태 파일 목록 (네임스페이스 폴더 대응)
...
**목표:** Hono RPC + TanStack Query로 서버 데이터 연결

### 8-1. `src/web/hooks/use-config.ts`

- [ ] Hono RPC 클라이언트 (`hc<AppType>`) 설정
  - Bearer 토큰 자동 첨부
- [ ] `useOverview()` — 대시보드 데이터
- [ ] `useClaudeMd(scope)` — 읽기 + mutation (저장)
- [ ] `usePlugins()` — 목록 + toggle mutation
- [ ] `useMcpServers()` — 목록 + add/remove mutation
- [ ] `useAgents()`, `useCommands()`, `useSkills()` — 목록 + CRUD mutation
- [ ] `useCliStatus()` — CLI 설치 여부
- [ ] 공통 Query 옵션: `refetchOnWindowFocus: true`, `refetchInterval: 5000`

### 완료 기준
- React DevTools에서 Query 캐시 확인
- 서버 데이터가 프론트엔드에 정상 표시
...
- [ ] Hono 서버 시작 (`127.0.0.1:4321`)
  - 포트 충돌 시 4322, 4323... 순차 탐색
...
- [ ] Hono 서버: 프로덕션 모드에서 `dist/web/` 정적 파일 서빙
...
### 12-1. 서버 단위 테스트
- [ ] config-service 전체 함수 (Phase 4에서 작성)
- [ ] file-writer 전체 함수 (Phase 5에서 작성)
- [ ] claude-cli mock 테스트 (Phase 5에서 작성)
- [ ] auth 미들웨어 테스트 (Phase 3에서 작성)
